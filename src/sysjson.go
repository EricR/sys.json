...
var (
	...
	modules = map[string]func() j{
		...
		"process": func() j {
			processes := proc.GetProcessTree().Map(func(fm proc.FieldMap, key string, val proc.FieldMap) {
				fm[key] = j{
					"name":         val["name"],
					"cmdline":      val["cmdline"],
					"state":        val["state"],
...
func statsHandler(w http.ResponseWriter, r *http.Request) {
	startTime := time.Now()
	paramModules := r.URL.Query().Get("modules")
	...
	if *debug {
		log.Printf("[debug] Handled request: %s (%fs)", r.RequestURI, time.Now().Sub(startTime).Seconds())
	}
